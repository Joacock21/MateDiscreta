function openRightPanel() { document.getElementById('rightPanel').classList.add('active'); }
...
function showSection(sectionName){
    ...
    if(sectionName === 'variables'){
        sectionTitle.textContent = '📊 8 Variables'; 
        ...
        <div style="margin-bottom: 2rem; display: flex; gap: 1rem; flex-wrap: wrap; align-items: center;">
            ...
            <button onclick="pedirLinkGoogleSheets()" class="add-btn" style="background: linear-gradient(135deg, #f59e0b, #d97706); font-size: 0.9rem; padding: 0.8rem 1.2rem;">☁️ Importar Google Forms</button>
        ...
        window.variables.forEach(variable => {
            html += `<div class="variable-card variable-item" onclick="openVariablePanel(${variable.id})"><h3>${variable.name}</h3><p>${variable.datos.length} datos guardados</p></div>`;
        }); 
        html += '</div>'; 
            ...
            <div class='plano-controls'>
                ...
                <div class='variable-selector'>
                    <div class='variable-select-card'>
                        ...
                        <p style='margin-top: 0.8rem; color: var(--gray-text); font-size: 0.85rem;'>📌 La variable que <strong>controlas</strong> o <strong>manipulas</strong></p>
                    ...
                    <div class='variable-select-card'>
                        ...
                        <p style='margin-top: 0.8rem; color: var(--gray-text); font-size: 0.85rem;'>📌 La variable que <strong>observas</strong> o <strong>mides</strong></p>
                        <p style='margin-top: 0.5rem; color: var(--gray-text);'>Datos: <span id='datosY'>-</span></p>
                    </div>
                </div>
                
                <div style='background: rgba(16, 185, 129, 0.1); border-left: 4px solid #10b981; padding: 1rem; border-radius: 8px; margin: 1rem 0;'>
                    <h4 style='color: #10b981; margin-bottom: 0.5rem; font-size: 1rem;'>💡 Ejemplos de Relaciones</h4>
                    <p style='color: var(--light-text); font-size: 0.9rem; line-height: 1.6;'>
                        📚 <strong>Horas estudio (X)</strong> → <strong>Nota promedio (Y)</strong>: Correlación positiva<br>
                        🏃 <strong>Horas ejercicio (X)</strong> → <strong>Peso corporal (Y)</strong>: Correlación negativa<br>
                        🚌 <strong>Días transporte (X)</strong> → <strong>Gasto S/ (Y)</strong>: Correlación positiva<br>
                        📱 <strong>Horas redes (X)</strong> → <strong>Horas sueño (Y)</strong>: Correlación negativa
                    </p>
                </div>
                ...
                <div style='display: flex; gap: 1rem; margin-top: 1.5rem; flex-wrap: wrap; align-items: center;'>
                    ...
                    <button class='add-btn' onclick='descargarGrafico()' style='background: linear-gradient(135deg, #7c3aed, #8b5cf6);'>🖼️ Descargar PNG</button>
                    <button class='clear-btn' onclick='clearPlano()'>🗑️ Limpiar</button>
                </div>
                
                <div style='background: rgba(37, 99, 235, 0.1); border-left: 4px solid #2563eb; padding: 1rem; border-radius: 8px; margin-top: 1rem;'>
                    <p style='color: var(--light-text); font-size: 0.9rem;'>
                        🖱️ <strong>Controles:</strong> Pasa el mouse sobre los puntos para ver valores exactos • Usa scroll para hacer zoom • Arrastra para mover el gráfico
                    </p>
                </div>
...
function openVariablePanel(varId){
    ...
    let html = `<div style='margin-bottom: 2rem;'><label style='color: var(--gray-text); font-size: 0.9rem; display: block; margin-bottom: 0.5rem;'>✏️ Nombre</label><input type='text' value='${variable.name}' onblur='updateVariableName(${varId}, this.value)' onkeypress='if(event.key==="Enter") {updateVariableName(${varId}, this.value); this.blur();}' style='background: rgba(30, 41, 59, 0.7); border: 2px solid var(--border-color); color: var(--accent-color); font-size: 1.4rem; width: 100%; padding: 0.8rem 1rem; border-radius: 10px; font-weight: 600;' placeholder='Nombre...'><p style='color: var(--gray-text); font-size: 0.8rem; margin-top: 0.5rem;'>💡 Enter para guardar</p></div>`;
    html += `<div class='form-section'><h3 style='color: var(--accent-color);'>➕ Agregar Dato</h3><input type='number' step='any' id='newDatoValue' placeholder='Valor numérico' onkeypress='if(event.key==="Enter") addDato(${varId})'><button class='add-btn' onclick='addDato(${varId})'>+ Agregar</button></div>`;
    html += `<div class='datos-list'><h3>Datos (${variable.datos.length})</h3>`;
    if (variable.datos.length === 0) { html += `<p style='text-align: center; color: var(--gray-text); padding: 2rem;'>📊 Sin datos</p>`; } 
    else { variable.datos.forEach((dato, index) => { html += `<div class='dato-item'><span>Dato ${index + 1}:</span><span class='dato-number'>${dato}</span><button class='delete-btn' onclick='deleteDato(${varId},${index})'>Eliminar</button></div>`; }); }
    html += '</div>';
...
function deleteDato(varId, i){ if(confirm('¿Eliminar?')) { deleteDatoFromVariable(varId, i); openVariablePanel(varId); } }
...
function updatePlano(){
    ...
    if(xId){ const vx = getVariable(xId); document.getElementById('datosX').textContent = vx.datos.length > 0 ? vx.datos.join(', ') : 'Sin datos'; }
    if(yId){ const vy = getVariable(yId); document.getElementById('datosY').textContent = vy.datos.length > 0 ? vy.datos.join(', ') : 'Sin datos'; }
    drawPlano();
}
...
function drawPlano() {
    ...
    // Calcular regresión con opción de forzar origen
    ...
    if(mostrarRegresion) {
        ...
        // Calcular puntos con padding reducido
...
function exportarExcel() {
    ...
    // Interpretación
    const absR = Math.abs(resultado.R);
    let interpretacionR = '';
    ...
    csv += '\nINTERPRETACIÓN\n';
    csv += `Tipo de correlación:,${interpretacionR} (${resultado.R > 0 ? 'positiva' : 'negativa'})\n`;
    csv += `Varianza explicada:,El ${(resultado.R2 * 100).toFixed(1)}% de la variación en ${varY.name} es explicada por ${varX.name}\n`;
...
function descargarGrafico() {
    ...
    showNotification('✅ Gráfico descargado');
}
...
function guardarCambiosJSON() {
    ...
    try {
        ...
        if (Array.isArray(nuevosDatos)) {
            ...
            document.body.style.overflow = 'auto';
        } else { alert('⚠️ Formato incorrecto (debe ser array).'); }
    } catch (e) { alert('❌ Error de Sintaxis JSON: ' + e.message); }
}
...
function formatearJSON() {
    ...
    } catch (e) { alert('⚠️ Error de sintaxis.'); }
}

function pedirLinkGoogleSheets() {
    const url = prompt("Link del formulario");
    if (url) importarDesdeGoogleSheets(url);
}